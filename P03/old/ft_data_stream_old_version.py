...
def ft_generate_events(data: list[dict[str, str | int]]):
    """ Produces an evet with name, level and ev. type"""
    for d in data:
        yield (f"Event {d["id"]}: Player {d["player"]} (level {d["data"]["level"]}) {d["event_type"]}" )


def ft_fibonacci_generator():
    """ produce the next number in the fibonnacci sequence """
    a: int = 0
    b: int = 1
    while True:
        yield a
...
def ft_prime_generator():
    """Generates prime numbers indefinitely"""
    num = 2
    while True:
        is_prime = True
        # Check factors until half the value
...
if __name__ == "__main__":
    ...
    # do everything at once, not only the print, also the analytics
    high_players: int = 0
    treasure_events: int = 0
    level_events: int = 0
    iter_events = ft_generate_events(data)
    for n in data:
        print(next(iter_events))
        if n["data"]["level"] >= 10:
            high_players += 1
        if n["event_type"] == "item_found":
            treasure_events += 1
        if n["event_type"] == "level_up":
            level_events += 1
    end = time.time()
    ...
    iter_fibonacci = ft_fibonacci_generator()
    for n in range(10):
        fibonacci_sequence.append(next(iter_fibonacci))
    print(f"Fibonacci sequence (first 10): {', '.join(map(str, fibonacci_sequence))}") # Can I do this????

    prime_sequence: list[int] = []
    iter_prime = ft_prime_generator()
    for n in range(5):
        prime_sequence.append(next(iter_prime))
